...
export default function App() {
  ...
  function Render() {
    return setNewPage((prevPage) => !prevPage), setLoading(true);
  }
  ...
  function checkAnswer() {
    return setShowAnswer((prevData) => !prevData);
  }
  ...
  React.useEffect(() => {
    ...
    for (let i = 0; i < question.length; i++) {
      if (question[i].selected_answer !== "undefined") {
        if (
          question[i].options[question[i].selected_answer] ===
    ...
    setQuestion((prevData) => {
      return question.map((question, id) => {
        return question_id === id
          ? { ...question, selected_answer: option_id }
          : question;
      });
    });
  }
        ...
        <div>
          ...
          {showAnswer ? (
            <div className="footer">
              ...
              <button className="playAgain" onClick={palyAgain}>
                play Again
              </button>
            </div>
          ) : (
